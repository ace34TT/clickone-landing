  ...
  return (
    <BlockContainer id={"tarifs"} className={"vertical-gradient-bg"} styles={{isFullWidth: true}}>
        ...
        {plans && plans.length > 0 && (
          <div className={`grid grid-cols-1 md:grid-cols-2 ${plans.length === 3 ? 'lg:grid-cols-3' : ''} gap-6 mb-16 max-w-6xl mx-auto`}>
            {plans.map((plan: any, idx: number) => {
              ...
              return (
                <div
                  key={idx}
                  className={`relative flex flex-col rounded-4xl px-8 py-12 md:px-10 md:py-16 min-h-[500px] lg:min-h-[620px] shadow-2xl overflow-hidden ${bgColorClass}`}
                  style={inlineStyle}
                >
