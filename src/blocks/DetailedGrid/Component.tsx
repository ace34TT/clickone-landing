...
> = (props) => {
  ...
  const TextBlock = ({ item, idx }: { item: any; idx: number }) => (
    <div key={idx} className="flex flex-col relative group">
      ...
      <div className="w-[60%] lg:w-[75%] h-[1px] bg-[#00e5ff] ml-auto opacity-70 group-hover:opacity-100 transition-opacity" />
    </div>
  )
            ...
            {imageArray && imageArray.length > 0 && (
              <div className="flex items-center justify-evenly lg:justify-between gap-4 mt-auto pt-8">
                {imageArray.map((img: any, idx: number) => (
                  <div key={idx} className="relative w-24 h-24 md:w-28 md:h-28 lg:w-32 lg:h-32 flex-shrink-0 transition-transform hover:scale-105 duration-300">
