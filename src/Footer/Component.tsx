...
export async function Footer() {
  ...
  return (
    <footer className="relative bg-[#0B1221] text-white pt-24 pb-12 overflow-hidden mt-auto border-t border-slate-800">
      
      {/* Decorative Dotted Mesh Left */}
      <div className="absolute left-[-2rem] top-1/2 -translate-y-1/2 w-48 lg:w-64 h-64 pointer-events-none opacity-60 hidden md:block">
        <svg fill="none" viewBox="0 0 100 200" className="w-full h-full text-[#00e5ff] opacity-80">
          ...
          <rect x="0" y="0" width="100" height="200" fill="url(#dots-left)" mask="url(#tri-mask-left)" />
        </svg>
      </div>

      {/* Decorative Dotted Mesh Right */}
      <div className="absolute right-[-2rem] top-1/2 -translate-y-1/2 w-48 lg:w-64 h-64 pointer-events-none opacity-60 hidden md:block">
        <svg fill="none" viewBox="0 0 100 200" className="w-full h-full text-[#00e5ff] opacity-80">
          ...
          <rect x="0" y="0" width="100" height="200" fill="url(#dots-right)" mask="url(#tri-mask-right)" />
        </svg>
      </div>
      ...
      <div className="container mx-auto px-4 md:px-8 max-w-5xl relative z-10 flex flex-col items-center">
        ...
        <div className="text-center mb-10">
          <h2 className="text-5xl md:text-7xl lg:text-[5rem] font-bold tracking-tight leading-tight md:leading-[1.15]">
            ...
            <span className="text-[#00e5ff]">{titleHighlight || 'Votre Projet'}</span> {titleEnd || 'ensemble'}
          </h2>
        </div>

        {/* Smile Icon SVG */}
        <div className="mb-20 text-[#00e5ff]">
          <svg width="64" height="40" viewBox="0 0 48 32" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M4 14C4 14 12 28 24 28C36 28 44 14 44 14" stroke="currentColor" strokeWidth="6" strokeLinecap="round" strokeLinejoin="round"/>
            <path d="M10 6H18" stroke="currentColor" strokeWidth="6" strokeLinecap="round"/>
            <path d="M36 8C36.0001 8 36.0002 8 36.0002 8" stroke="currentColor" strokeWidth="6" strokeLinecap="round"/>
          </svg>
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-center lg:justify-between w-full gap-12 lg:gap-8 mb-20 text-center">
          ...
          <div className="flex-1 flex flex-col items-center gap-6">
            ...
            <span className="text-xl md:text-2xl font-bold tracking-wider">{phone || '07 85 76 30 86'}</span>
          </div>
          ...
          <div className="flex-1 flex flex-col items-center gap-6">
            ...
            <span className="text-xl md:text-2xl font-bold tracking-wider">{email || 'medji@click-one.fr'}</span>
          </div>
          ...
          <div className="flex-1 flex flex-col items-center gap-6">
            ...
            <span className="text-xl md:text-2xl font-bold tracking-wider">{website || 'click-one.fr'}</span>
          </div>
        </div>
        ...
        <div className="mb-28">
          {cta?.link ? (
            ...
            >
              ...
              <svg className="ml-2 w-5 h-5 font-bold" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7"></path></svg>
            </CMSLink>
          ) : (
